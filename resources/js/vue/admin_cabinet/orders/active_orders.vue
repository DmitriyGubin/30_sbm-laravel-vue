...
export default {
    props: ['active_orders','count_act_or','count_pages_act_or','cur_page_act_or'],
    components: {
        Order,Pager
    }
}
...
<template>
    <div v-if="count_act_or==0" class="empty_box">
    ...
    <Order v-else v-for="order in active_orders"
    :key = "order.id"
    :order = "order"
    type = "active"
    />

    <Pager v-if="count_pages_act_or > 1 && count_act_or != 0"
        :count_pages = "count_pages_act_or"
        :cur_page = "cur_page_act_or"
        cur_tab="active_orders"
    />
</template>
