...
export default {
    props: ['managerss','count_manrs','count_pages_manrs','cur_page_manrs'],
    components: {
        Pager,User
    }
}
...
<template>
    <div v-if="count_manrs==0" class="empty_box">
    ...
    <User v-else v-for="user in managerss"
    :key = "user.id"
    :elem = "user"
    type = "manager"
    />

    <Pager v-if="count_pages_manrs > 1 && count_manrs != 0"
        :count_pages = "count_pages_manrs"
        :cur_page = "cur_page_manrs"
        cur_tab="managers"
    />
</template>
