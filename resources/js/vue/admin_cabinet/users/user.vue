...
import Popup from './change_popup.vue';

export default {
    props: ['elem','type'],
    inject: ['Make_Activ','Delete_User'],
    components: {
        Popup
    }
}
...
<template>
    ...
    <div class="pers_item">
        <div class="phone_name">
            <svg class="icon_perse" width="40" height="40" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="phone_name">
                <span class="bold">{{ elem.name }}</span>
                <span>{{ elem.phone }}</span>
            </div>
        </div>

        <div class="butt_box">
            <button v-if="elem.activity == false" class="univ_button" @click="Make_Activ(elem.id, type)">
                Активировать
            </button>
            <a class="univ_button" data-fancybox="" :data-src="'#user_popup_'+elem.id" href="javascript:;">
                Изменить
            </a>
            <button @click="Delete_User(elem.id, type)" class="univ_button">
                Удалить
            </button>
        </div>
    </div>
