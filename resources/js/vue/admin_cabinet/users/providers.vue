...
export default {
    props: ['providerss','count_provs','count_pages_provs','cur_page_provs'],
    components: {
        Pager,User
    }
}
...
<template>
    <div v-if="count_provs==0" class="empty_box">
    ...
    <User v-else v-for="user in providerss"
    :key = "user.id"
    :elem = "user"
    type = "provider"
    />

    <Pager v-if="count_pages_provs > 1 && count_provs != 0"
        :count_pages = "count_pages_provs"
        :cur_page = "cur_page_provs"
        cur_tab="providers"
    />
</template>
