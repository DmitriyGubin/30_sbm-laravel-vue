...
import Rekviz_Popup from './rekviz_popup.vue';
import Rekviz_Item from './rekviz_item.vue';

export default {
    props: ['rekv','cur_recv_id'],
    emits: ['Add_Rekv','Delete_Rekv','Change_Cur_Rekv'],
    components: {
        Rekviz_Popup,Rekviz_Item
    },
    data() {
        return {
            add_item: {id: '', name: '', inn: '', kpp: '', bik: '', check: '', 	bank_name: '', file_name: ''},
        }
    },
    computed: {
        count(){
            return this.rekv.length;
        }
    },
    methods: {
        Clear_Add_Item(){
            for (let key in this.add_item)
            {
                this.add_item[key] = '';
            }
        }
...
<template>
    <a data-fancybox="popup" data-src="#rekviz_form" class="univ_button add_recv_butt" href="javascript:;">
        Добавить реквизиты
    </a>
    ...
    <Rekviz_Item v-else v-for="item in rekv"
    :key="item.id"
    :item="item"
    :cur_recv_id="cur_recv_id"
    @Delete_Rekv="$emit('Delete_Rekv',item.id, item.file_name)"
    @Change_Cur_Rekv="$emit('Change_Cur_Rekv',item.id)"
    />

    <Rekviz_Popup ref="popup"
    @Add_Rekv="$emit('Add_Rekv',add_item)"
    :add_item="add_item"
    @Clear_Add_Item="Clear_Add_Item"
    @Change_Cur_Rekv="$emit('Change_Cur_Rekv',this.add_item.id)"
    />
</template>
