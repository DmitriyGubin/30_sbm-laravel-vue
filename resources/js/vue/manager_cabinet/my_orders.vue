...
export default {
    props: ['my_orders','count_my_or','count_pages_my_or','cur_page_my_or'],
    ...
    methods: {
        ...
        Get_Sectss: function () {
            axios.post('/tech_sect_list').then(res => {
                ...
                {
                    this.all_sects.push(res.data[key]);
                }
            });
        }
    },
...
<template>
    ...
    <div v-if="count_my_or==0" class="empty_box">
    ...
    <Order v-else v-for="order in my_orders"
    :key="order.id"
    :order="order"
    :neww="false"
    :my="true"
    ...
    @Change_Status="$emit('Change_Status', order, 2)"
    @Delete_Order="$emit('Delete_Order', order.id)"
    />

    <Pager v-if="count_pages_my_or > 1 && count_my_or != 0"
        :count_pages = "count_pages_my_or"
        :cur_page = "cur_page_my_or"
        cur_tab="my_orders"
    />
