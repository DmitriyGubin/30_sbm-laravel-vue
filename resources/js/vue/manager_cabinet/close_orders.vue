...
export default {
    props: ['close_orders','count_close_or','count_pages_close_or','cur_page_close_or'],
...
<template>
    ...
    <div v-if="count_close_or==0" class="empty_box">
    ...
    <Order v-else v-for="order in close_orders"
    :key="order.id"
    :order="order"
    :neww="false"
    :my="false"
    :close="true"
    @Take_Order="$emit('Take_Order', order)"
    @Change_Status="$emit('Change_Status', order, 1)"
    @Delete_Order="$emit('Delete_Order', order.id)"
    />
    
    <Pager v-if="count_pages_close_or > 1 && count_close_or != 0"
        :count_pages = "count_pages_close_or"
        :cur_page = "cur_page_close_or"
        cur_tab="close_orders"
    />
