...
import Supp_Popup from './supplier_popup.vue';

export default {
    components: {
        Tech_Popup,Supp_Popup
    },
    inject: ['Refuse_Order'],
    props: ['order','neww','my','close','providers','all_sects'],
    emits: ['Take_Order','Change_Status','Delete_Order'],
    data() {
        return {
            hide: false,
            edit: false,
            save: false,
            send_mess: this.order.find_supplier,
            loader_check: false,
            error_prov: false
        }
    },
    methods: {
        ...
        Сhoice_Tech: function (tech_name) { 
            ...
            if(this.all_cond) this.save = true;
        },
        Show_Save: function () { 
            if(this.all_cond)
            {
        ...
        Ready_Masseges: function() {
            this.send_mess = 1;
        },
        Auto_Send: function(tech_name)
        {
            ...
            axios.post('/auto_send_provider', post_data).then(res => {
               this.loader_check = false;
               if(res.data == 'no')
               {
                this.error_prov = true;
               }
               else
               {
                this.error_prov = false;
               }
...
<template>
    ...
    <div class="order_item form_box">
        ...
        <h2 class="order_title" v-show="send_mess == 1">Сообщения отправлены клиентам</h2>
        ...
        <div class="button_line">
            ...
            <template v-if="my">
                <template v-if="!save">
                    ...
                    <a data-fancybox="" :data-src="'#supplier_popup'+'_'+order.id" class="univ_button" href="javascript:;">
                        Найти поставщика
                    </a>
    
                    <button @click="Auto_Send(order.tech)" class="univ_button auto_sending">
                        <span v-if="!loader_check">Разослать всем</span>
            ...
            <button v-show="!edit" @click="$emit('Change_Status', order, 2)" v-if="my" class="univ_button">
                Завершить
            </button>

            <button v-show="!edit" v-if="my" @click="Refuse_Order(order)" class="univ_button">
                Отказаться
            </button>

            <button @click="$emit('Change_Status', order, 1)" v-if="close" class="univ_button">Вернуть в работу</button>
            <button style="display: none;" @click="$emit('Delete_Order', order.id)" v-if="close" class="univ_button">Удалить</button>
            <button @click="$emit('Take_Order', order)" v-if="neww" class="univ_button">
                Обработать заявку
            </button>
        
        </div>

        <div class="error_text white" v-show="my && error_prov">
            Под такую технику не нашлось поставщиков, попробуйте найти поставщика вручную (кнопка "Найти поставщика")
        </div>
        ...
        <div v-show="hide">
            <div class="hide_box">
                ...
                <div class="client_box" v-if="order.provider != null">
                    <div class="client_name">Нашёлся поставщик: {{ order.provider.name }}</div>
