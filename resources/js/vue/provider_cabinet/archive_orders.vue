...
export default {
    props: ['archive_orders'],
    emits: ['Return_To_Work'],
    components: {
        Order
    }
}
...
<template>
    <div v-if="archive_orders.length == 0" class="empty_box">
    ...
    <Order v-else v-for="order in archive_orders"
    :key="order.id"
    :order="order"
    :my="false"
    :arch="true"
    @Return_To_Work="$emit('Return_To_Work', order)"
    />
</template>
